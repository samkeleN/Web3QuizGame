    ...
    const { seedPhrase } = await inquirer.prompt([
      {
        ...
        name: "seedPhrase",
        message:
          "Enter your Farcaster custody account seed phrase to sign the frame manifest\n(optional -- leave blank to create an unsigned frame)\n\nSeed phrase:",
        default: null,
      },
    ]);
...
async function deployToVercel(useGitHub = false) {
  try {
    ...
    console.log(
      " An initial deployment is required to get an assigned domain that can be used in the frame manifest\n",
    );
    console.log(
      ...
      if (currentProject) {
        ...
        if (domainMatch) {
          ...
          if (actualDomain !== domain) {
            ...
            console.log(
              "🔄 Updating environment variables with correct domain...",
            );
...
async function main() {
  try {
    ...
    console.log(
      "4. Deploy and build your frame (Vercel will run the build automatically)\n",
    );
